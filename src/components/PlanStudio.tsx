...
type SceneCommand = 'top' | 'perspective' | 'view-a' | 'view-b' | 'view-c' | 'view-d' | 'fit' | 'pan-left' | 'pan-right' | 'pan-up' | 'pan-down' | 'zoom-in' | 'zoom-out' | 'capture' | 'snapshot'
...
export default function PlanStudio() {
  ...
  const [shellView, setShellView] = useState<'top' | 'perspective' | CameraView>('perspective')
  ...
  const buildScene = () => {
    ...
    setSceneSegments(segments.map((segment) => ({ ...segment }))); setDimensionChecks(createDimensionChecks(segments, crop, planWidth, wallHeight, wallThickness)); setGeometryLocked(false); setSceneReady(false); setCameraView('A'); setShellView('perspective'); setCommand((value) => ({ id: value.id + 1, type: 'perspective' })); setActiveStep(2); setStatus('Editable shell ready · validate dimensions')
  }
  ...
  const onSnapshot = useCallback(async (imageDataUrl: string) => {
    ...
    setCommand((value) => ({ id: value.id + 1, type: `view-${cameraView.toLowerCase()}` as SceneCommand }))
  ...
  const returnToStyle = () => {
    setShellView('perspective')
    setCommand((value) => ({ id: value.id + 1, type: 'perspective' }))
    setActiveStep(3)
    setStatus('Style settings open · completed renders retained')
    notify('Style opened · existing renders are retained')
