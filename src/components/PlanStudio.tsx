...
export default function PlanStudio() {
  ...
  const activeRender = useMemo(() => renderedViews.find((item) => item.id === activeRenderId) ?? renderedViews[0], [activeRenderId, renderedViews])
  ...
  const analyseFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setSceneSegments([]); setSceneObjects([]); setSceneReady(false); setRenderedViews([]); setActiveRenderId(''); setDimensionChecks([]); setGeometryLocked(false)
  ...
  const buildScene = () => {
    ...
    setSceneSegments(segments.map((segment) => ({ ...segment }))); setDimensionChecks(createDimensionChecks(segments, crop, planWidth, wallHeight, wallThickness)); setGeometryLocked(false); setSceneReady(false); setCameraView('A'); setCommand((value) => ({ id: value.id + 1, type: 'view-a' })); setActiveStep(2); setStatus('Editable shell ready · validate dimensions')
  }
  ...
  const chooseCameraView = (view: CameraView) => { setCameraView(view); sendCommand(`view-${view.toLowerCase()}` as SceneCommand) }
    ...
    try {
      ...
      for (let attempt = 0; attempt < 48; attempt += 1) {
        ...
        if (result.status === 'COMPLETED') {
          ...
          setRenderedViews((items) => [...completed, ...items]); setActiveRenderId(completed[0].id); setComparison(100); setStatus(`${CAMERA_NAMES[cameraView]} · Rev ${revision} ready`); notify(`${completed.length} variant${completed.length > 1 ? 's' : ''} completed`); return
        }
      ...
      const message = error instanceof Error ? error.message : 'Rendering failed'; setRenderError(message); setStatus('Photorealistic rendering needs attention'); notify(message)
    } finally { setRendering(false) }
  }, [cameraView, geometryLocked, notify, renderAccessCode, renderPrompt, renderedViews, variantCount])
  ...
  const reset = () => { setStudio(false); setSceneSegments([]); setSceneObjects([]); setSceneReady(false); setRenderedViews([]); setActiveRenderId(''); setRenderError(''); setDimensionChecks([]); setGeometryLocked(false); setActiveStep(1); setStatus('Ready for a layout plan'); setImage(null); setRaster(null); setSegments([]) }
  ...
  return <>
    ...
    <main>
      {!studio ? <section className="upload-view"><div className="upload-copy"><p className="eyebrow">Geometry-controlled AI visualisation</p><h1>From layout drawing to validated interior concept.</h1><p className="lede">Upload a floor plan, review the detected geometry, validate key dimensions, then generate styled photorealistic views from a locked 3D shell.</p><div className="truth-note"><strong>Accuracy you can audit</strong><span>Renderline reports the pass rate only for measurements explicitly reviewed against the source plan.</span></div></div><div className={`upload-card ${busy ? 'loading' : ''}`} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const file = event.dataTransfer.files[0]; if (file) analyseFile(file) }}><input ref={fileRef} type="file" accept=".pdf,.png,.jpg,.jpeg,.webp" hidden onChange={(event) => { const file = event.target.files?.[0]; if (file) analyseFile(file) }} /><div className="upload-icon">↑</div><h2>Upload layout plan</h2><p>PDF, PNG, JPG or WEBP · processed locally</p><button className="primary" disabled={busy} onClick={() => fileRef.current?.click()}>{busy ? 'Analysing…' : 'Choose plan'}</button><div className="or"><span />or<span /></div><button className="sample" disabled={busy} onClick={async () => { const response = await fetch(`${BASE_PATH}/sample/Hui-Ting-Layout.pdf`); analyseFile(new File([await response.blob()], 'Hui Ting Layout.pdf', { type: 'application/pdf' })) }}>Use supplied Hui Ting layout</button></div></section> : <>
        <nav className="project-nav"><div><strong>{projectName.replace(/\.[^.]+$/, '')}</strong><small>{geometryLocked ? `Geometry locked · ${accuracy.score}% of confirmed checks passed` : 'Geometry review in progress'}</small></div><ol>{STEPS.map(([label, note], index) => <li key={label} className={`${activeStep === index + 1 ? 'active' : ''} ${activeStep > index + 1 ? 'done' : ''}`}><button onClick={() => setActiveStep(index + 1)}><span>{activeStep > index + 1 ? '✓' : index + 1}</span><div><strong>{label}</strong><small>{note}</small></div></button></li>)}</ol></nav>
